...
const JourneyScreen: React.FC<JourneyScreenProps> = ({
  ...
  const handleShareAchievement = useCallback(async (template: ShareTemplate) => {
    ...
    // Template'in render edilmesi icin kisa bekleme
    setTimeout(async () => {
      try {
        ...
        <View style={styles.shareRow}>
          {currentStreak >= 3 && (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
              disabled={isSharing}
              onPress={() => handleShareAchievement(createStreakTemplate(currentStreak))}
            >
              <Text style={styles.shareBtnText}>
                {'\u{1F525}'} Share Streak
              </Text>
            </TouchableOpacity>
          )}
          {totalWordsLearned >= 5 && (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
              disabled={isSharing}
              onPress={() => handleShareAchievement(createWordsTemplate(totalWordsLearned))}
            >
              <Text style={styles.shareBtnText}>
                {'\u{1F4DA}'} Share Words
              </Text>
